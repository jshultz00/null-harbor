...
#   - Use cr_* scripts (from COMMANDLY_SERVER) for all remote machine interactions
...
# Set attacker IP alias for this phase (within 9.53.99.0/24)
# For IPs outside this range, use SNAT at fw-dmz instead:
#   cr_runcmd.bash fw-dmz "nft flush chain ip nat SCENARIO_SNAT"
#   cr_runcmd.bash fw-dmz "nft add rule ip nat SCENARIO_SNAT ip saddr 9.53.99.1 oifname eth2 snat to ${ATTACKER_IP_PHASE1}"
...
# source /usr/local/bin/cr_runcmd.bash  (or use full path)
# cr_runcmd.bash "${TARGET_WEB_LIN}" "id"

# Example: copy a file to a remote machine
# cr_copytoremote.bash "${TARGET_WEB_LIN}" "${SCRIPT_DIR}/../attacker_files/payload.py" /tmp/payload.py

# Example: check if a file exists on a remote machine (returns 0/1)
# cr_checkfile.bash "${TARGET_WEB_LIN}" /tmp/payload.py 10
