...
# Usage:  runcmdForSim.bash hn-wks-01 "ls -la /tmp"
...
if [[ -z "${1}" || -z "${2}" ]]; then
    echo ""
    echo "Usage: runcmdForSim.bash hostname command [runAsUser] [runAsPassword] [runAsDomain]"
    echo ""
    echo 'Example: runcmdForSim.bash hn-wks-01 "ls -la /tmp"'
    echo 'Example with run-as: runcmdForSim.bash hn-wks-01 "myapp.exe" myuser mypassword'
    echo ""
...
JSON_PAYLOAD=$(printf '{"hostname":"%s","key":"simRun","value":"%s","debug":"true"%s}' \
